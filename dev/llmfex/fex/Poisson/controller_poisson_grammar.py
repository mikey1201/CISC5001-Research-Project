...
This is a modified version of controller_poisson.py that supports
pluggable grammar sources (FEX default, LLM-generated, or hybrid).

Usage:
    # Default FEX grammar (backward compatible)
    python controller_poisson_grammar.py --dim 20 --epoch 2000

    # LLM-generated grammar
    python controller_poisson_grammar.py --dim 20 --epoch 2000 \
        --grammar_source llm --grammar_path path/to/grammar.json

    # Hybrid grammar (FEX + LLM)
    python controller_poisson_grammar.py --dim 20 --epoch 2000 \
        --grammar_source hybrid --grammar_path path/to/grammar.json
"""
...
# Import PDE-specific functions (these remain unchanged for backward compatibility)
...
parser = argparse.ArgumentParser(description='FEX with Grammar Abstraction')
...
if args.problem_config and os.path.exists(args.problem_config):
    import problem_config as prob_cfg
    prob_cfg.load_config(args.problem_config)
    print(f"Loaded problem config: {args.problem_config}")
    cfg = prob_cfg.get_config()
    if cfg:
        print(f"  Problem ID: {cfg.get('problem_id', 'unknown')}")
        print(f"  PDE Type: {cfg.get('pde_type', 'unknown')}")
        print(f"  RHS: {cfg.get('rhs', 'default')[:50]}..." if cfg.get('rhs') else "  RHS: default")
        print(f"  Solution: {cfg.get('solution', 'default')[:50]}..." if cfg.get('solution') else "  Solution: default")
...
if __name__ == '__main__':
    print("=" * 80)
    print("FEX with Grammar Abstraction")
    print("=" * 80)
